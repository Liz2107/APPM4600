...
def driver():
    ...
    # print("time of iteration ", elapsed/50)
    # print("ier is ", ier	)
...
def evalF(x): 
    ...
    F[1] = (1-x[0])**0.25 - x[1] + 0.05*x[2]**2 - .15*x[2] - 1
    F[2] = -x[0]**2 - 0.1*x[1]**2 + .01*x[1] + x[2] - 1
    return F
...
def Hybrid(x,tol,Nmax):
    ...
    for its in range(Nmax):
        ...
        if abs(gval - g1)<(5*10**-2):
            
            return Newton(x, tol, Nmax)
            # ier = 0
            # return [x,gval,ier]
